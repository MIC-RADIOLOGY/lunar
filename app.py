...
if uploaded_file:
    ...
    # Convert numeric-looking columns to numeric (ignores text)
    ...
    if len(detected) == 0:
        ...
        for bucket, column in detected.items():
            numeric_col = pd.to_numeric(df[column], errors="coerce")  # Convert to numeric, coerce errors to NaN
            totals[bucket] = numeric_col.sum()
        ...
        if "Current" in percentages:
            st.write(f"• {percentages['Current']:.1f}% of receivables fall within the current period, indicating relatively healthy collections.")

        if "30 Days" in percentages:
            st.write(f"• {percentages['30 Days']:.1f}% of receivables fall within the 30-day category.")

        if "60 Days" in totals and "90 Days" in totals:
            mid_term = totals["60 Days"] + totals["90 Days"]
            mid_pct = (mid_term / total_balance) * 100
            st.write(f"• Accounts between 60–90 days total ${mid_term:,.2f} ({mid_pct:.1f}% of receivables).")

        long_term = totals.get("120 Days", 0) + totals.get("150 Days", 0) + totals.get("180+ Days", 0)
        if long_term > 0:
            long_pct = (long_term / total_balance) * 100
            st.write(f"• Accounts older than 120 days total ${long_term:,.2f} ({long_pct:.1f}% of receivables).")
